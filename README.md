...
+ `db/`: contain our modified database, which supports compression in the MemTable and testing of CMS in decompression-free compaction
    +  `CompressedSeriesTest`: test the correctness of using encoding in MemTable, whether using the compressed MemTable or not can be set in `IoTDBConfig`
    +  `InnerUnseqCompactionWithUndecodePerformerTest`: test the correctness of using CMS in decompression-free compaction of two given tsfiles, which can be generated by `TsFilegenerator`
