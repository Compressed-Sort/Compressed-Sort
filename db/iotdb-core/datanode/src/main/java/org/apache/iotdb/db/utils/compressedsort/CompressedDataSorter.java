...
public class CompressedDataSorter {
  ...
  public int persistentSortOpeHoldLen(
      ...
      long valueTemp,
      int valueTempPos,
      int valueValueTempPos,
      TS_DELTA_decoder timeForwardDecoder,
      V_VARINT_decoder valueForwardDecoder,
      int blockBegInd,
      int blockEndPos) {
    // 长度不变的压缩数据的一次基本操作(减少数据整体移动的开销)
    long upBound = nowValue;
    this.timeBackwardDecoder.reset(
        timeForwardDecoder.nowValue, timeForwardDecoder.nowNum, timeForwardDecoder.nowPos);
    this.valueBackwardDecoder.reset(
        valueForwardDecoder.nowValue, valueForwardDecoder.nowNum, valueForwardDecoder.nowPos);
    while (valueBackwardDecoder.backwardDecode(this.compressedValueData) > Long.MIN_VALUE
        && timeBackwardDecoder.backwardDecode(this.compressedTimeData) > valueTemp) {
    ...
    int valueTempVar =
        encoder.changeEncodeWithFixedLenByInd(
            valueTempInd,
            valueTempPos,
            valueTemp - timeBackwardDecoder.nowValue,
            timeBackwardDecoder.nowNum,
            compressedTimeData,
            timeForwardDecoder.nowPos);
    int upBoundVar;
    if (newHeadPos < blockEndPos) {
      upBoundVar =
          encoder.changeEncodeWithFixedLenByInd(
    ...
    if (newHeadPos < blockEndPos) {
      ...
      newHeadVar =
          encoder.changeEncodeWithFixedLen(
              timeForwardDecoder.nowNum - 1,
              newHeadPos,
              timeForwardDecoder.nowValue - nowValue,
              8,
              compressedTimeData,
              timeForwardDecoder.nowPos);
    } else {
    ...
    lenDataMove(
        compressedTimeData.lens,
        timeBackwardDecoder.nowNum,
        valueTempInd,
        timeForwardDecoder.nowNum - 1);
    lenDataMove(
        compressedValueData.lens,
        valueBackwardDecoder.nowNum,
        valueTempInd,
        valueForwardDecoder.nowNum - 1);
