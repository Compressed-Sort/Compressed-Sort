...
public class TS_DELTA_sorter implements Runnable {
  ...
  public void run() {
    ...
    // System.out.println("Thread " + Thread.currentThread().getId() + " updated index " + index + "
    // to " + array[index]);

  }

  public int sort(
      int begIndex, int endIndex, int begPos, int endPos) { // 返回endPos的变化情况，用 newEndPos - oldEndPos
    // 试图拆分成两个不相交的子问题
    // 如果没有点或者只有一个点，直接结束
    ...
    block2ndVar +=
        encoder.changeEncode(
            midIndex, midPos, block2ndMinVal - block1stMaxVal, compressedData, endPos);
    // 将两部分的数据整理合并
  ...
  public int sort2(
    ...
    if (midIndex == begIndex)
      return blockSort(begIndex, endIndex, begPos, endPos) - endPos; // 已经是一个有序的子问题
    ...
    TS_DELTA_encoder encoder = new TS_DELTA_encoder();
    TS_DELTA_data compressedData = new TS_DELTA_data(this.vals, this.lens);
    // block2ndVar += encoder.changeEncode(midIndex, midPos, block2ndMinVal-block1stMaxVal,
    // compressedData, endPos);  // change!
    block2ndVar +=
        encoder.changeEncodeWithFixedLen(
            midIndex, midPos, block2ndMinVal - block1stMaxVal, 4, compressedData, endPos);
    // 将两部分的数据整理合并
    mergeBlock(midPos + block1stVar, midPos, endPos);
    int blockVar = 0;
    if (block1stMaxVal > block2ndMinVal) {
      blockVar =
          blockSort(
                  ...
                  endIndex,
                  begPos,
                  endPos + block1stVar + block2ndVar,
                  block1stMaxVal,
                  block2ndMinVal,
                  midIndex,
                  midPos + block1stVar)
              - (endPos + block1stVar + block2ndVar);
      // blockVar = blockSort(begIndex, endIndex, begPos,
      // endPos+block1stVar+block2ndVar)-(endPos+block1stVar+block2ndVar);
    }
  ...
  public int blockSort(int begIndex, int endIndex, int begPos, int endPos) {
    ...
    while (true) {
      ...
      if (temVal >= nowVal || nowInd == begIndex) {
        ...
        // 找到一个乱序点
        endPos +=
            persistentSortOpeHoldLen(
                nowVal, nowValPos, temVal, temValPos, decoder, begIndex,
                endPos); // change!修改基本模块的运行逻辑
  ...
  public int findBestMidIndex(int begIndex, int endIndex, int begPos, int endPos) {
    ...
    for (int i = 0; i < endIndex - begIndex; i++) {
      if (isBlock2stMinVal[i] && isBlock1stMaxVal[i]) {
        if (Math.abs(i - (endIndex - begIndex) / 2)
            < Math.abs(midIndex - (endIndex - begIndex) / 2)) {
          midIndex = i;
        }
      }
    }
    ...
    while (decoder.nowPos > begPos) {
      ...
      if (true) {
        ...
        if (decoder.nowValue > block2ndMinVal) { // 判断当前点是否是乱序点
          if (Math.abs(decoder.nowNum - begIndex - (endIndex - begIndex) / 2)
              < Math.abs(midIndex - (endIndex - begIndex) / 2)) {
  ...
  public void findMidIndexWithPosAndValue(
      int begIndex, int endIndex, int begPos, int endPos, long[] out) {
  ...
  public int persistentSortOpe(
      long nowValue,
      int nowValuePos,
      long valueTemp,
      int valueTempPos,
      TS_DELTA_decoder forwardDecoder,
      int blockBegInd,
      int blockEndPos) {
    // 持续压缩排序算法的一次基本操作
    ...
    TS_DELTA_decoder backwardDecoder =
        new TS_DELTA_decoder(forwardDecoder.nowValue, forwardDecoder.nowNum, forwardDecoder.nowPos);
    ...
    // 直接修改三个位置元素的delta
    TS_DELTA_encoder encoder = new TS_DELTA_encoder();
    TS_DELTA_data compressedData =
        new TS_DELTA_data(this.vals, this.lens); // delta值发生变化后，后面相应的指针也会发生变化
    int valueTempVar =
        encoder.changeEncode(
            valueTempInd,
            valueTempPos,
            valueTemp - backwardDecoder.nowValue,
            compressedData,
            blockEndPos);
    int upBoundVar =
        encoder.changeEncode(
            backwardDecoder.nowNum, upBoundPos, upBound - tailVal, compressedData, blockEndPos);
    int newHeadVar = 0;
    ...
    if (newHeadPos < blockEndPos) {
      ...
      newHeadVar =
          encoder.changeEncode(
              forwardDecoder.nowNum - 1,
              newHeadPos,
              forwardDecoder.nowValue - nowValue,
              compressedData,
              blockEndPos);
    } else {
      newHeadPos += upBoundVar + valueTempVar;
  ...
  public int persistentSortOpeHoldLen(
    ...
    TS_DELTA_data compressedData =
        new TS_DELTA_data(this.vals, this.lens); // delta值发生变化后，后面相应的指针也会发生变化
    int valueTempVar =
        encoder.changeEncodeWithFixedLenByInd(
            valueTempInd,
            valueTempPos,
            valueTemp - backwardDecoder.nowValue,
            backwardDecoder.nowNum,
            compressedData,
            forwardDecoder.nowPos);
    int upBoundVar;
    if (newHeadPos < blockEndPos) {
      upBoundVar =
          encoder.changeEncodeWithFixedLenByInd(
