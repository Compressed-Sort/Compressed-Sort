...
public class CompressedTVList extends Series { // change!
  ...
  public CompressedTVList(
      TSDataType dataType, TVList tvList) { // todo:根据不同的数据类型和配置设置不同的encode和decode
    this.dataType = dataType;
    this.statistics = Statistics.getStatsByType(dataType);
    this.timeOut = new PublicBAOS();
    this.valueOut = new PublicBAOS();
  ...
  public byte[] rightMoveBuffer(
      int remain, byte[] buffer, int startPos, int startOffset, int moveLen) {
  ...
  public void leftMoveBuffer(byte[] buffer, int startPos, int startOffset, int moveLen) {
    ...
    if (8 - startOffset <= 8 - leftStartOffset) {
      ...
      nowNew =
          (byte)
              ((buffer[startPos] << (startOffset - leftStartOffset))
      ...
      nowNew =
          (byte)
              ((buffer[startPos] >> (leftStartOffset - startOffset))
                  & ((1 << (8 - leftStartOffset)) - 1));
      buffer[leftStartPos] = (byte) (leftOld | nowNew);
    ...
    while (true) {
      ...
      if (timestamp != timestampNow) {
        ...
        if (lentemp < valueDataLen) {
          valueByteArray =
              rightMoveBuffer(
                  valueBufferTemp.remaining(),
                  valueByteArray,
                  valueRightPos,
                  valueRightOffset,
                  valueDataLen - lentemp);
          valueOut.setBuf(valueByteArray);
        }
        // 插入数据
        writeBuffer(timeByteArray, timeLeftPos, timeLeftOffset, timeDataLen, timeOutTemp.getBuf());
        writeBuffer(
            valueByteArray, valueLeftPos, valueLeftOffset, valueDataLen, valueOutTemp.getBuf());
        return;
      }
    }
  }
  ...
  public static boolean appendToTXT(String content) {
    ...
    try (BufferedWriter writer =
        new BufferedWriter(new FileWriter(filePath, true))) { // 第二个参数true表示追加模式
