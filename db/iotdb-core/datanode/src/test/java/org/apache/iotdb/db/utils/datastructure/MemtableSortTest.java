...
public class MemtableSortTest {
  ...
  public void SortMemory(
      ByteBuffer timeBuffer, ByteBuffer valueBuffer, boolean encoderType, boolean tvListType) {
  ...
  public void testSortTime() throws IOException {
    ...
    for (int r = 0; r < REAPEAT_NUM; r++) {
      ...
      while (startIndex + PAGE_SIZE <= ROW_NUM) {
        ...
        CompressedDataSorter sorter =
            new CompressedDataSorter(timeCompressedData, valueCompressedData);
    ...
    writeDataToTXT(
        new long[] {
          sortNum,
          newTime / sortNum,
          TS_2DIFF_TimTime / sortNum,
          Chimp_TimTime / sortNum,
          TS_2DIFF_BackTime / sortNum,
          Chimp_BackTime / sortNum
        });
  }
  ...
  public void testEncodeDecodeTime() throws IOException {
    ...
    writeDataToTXT(
        new long[] {
          sortNum,
          totalTime_TS_DELTA / sortNum,
          totalTime_TS_2DIFF / sortNum,
          totalTime_CHIMP / sortNum
        });
  }
  ...
  public void prepareData(long[] times, long[] values) {
    ...
    readCSV(
        "D:/senior/DQ/research/compressed_sort_paper/dataset/real/swzl/swzl_clean.csv",
        2,
        ROW_NUM + 1,
        0,
        times);
    ...
    // readCSV("D:/senior/DQ/research/compressed_sort_paper/dataset/real/ship/shipNet.csv", 2,
    // ROW_NUM+1, 1, times);
    // readCSV("D:/senior/DQ/research/compressed_sort_paper/dataset/real/ship/shipNet.csv", 2,
    // ROW_NUM+1, 4, values);
  ...
  public void recordMemory(MemoryMXBean memoryMXBean) {
    ...
    writeDataToTXT(
        new long[] {heap.getUsed(), nonHeap.getUsed(), nonHeap.getCommitted(), nonHeap.getMax()});
  }
}
