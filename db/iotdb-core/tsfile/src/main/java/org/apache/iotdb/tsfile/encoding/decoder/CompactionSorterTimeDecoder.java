...
public class CompactionSorterTimeDecoder {
  long nowValue = 0;
  ...
  public CompactionSorterTimeDecoder(
      int nowIndex, long nowValue, LinkedList<CompressedPageData> sortedPageList) {
  ...
  public CompactionSorterTimeDecoder(LinkedList<CompressedPageData> sortedPageList) {
    this.data = sortedPageList;
  }

  public CompactionSorterTimeDecoder(int nowIndex, LinkedList<CompressedPageData> sortedPageList) {
    // 设置读的位置为sortedPageList里面第nowIndex位置的元素，并且指针指向第一个元素
    CompressedPageData nowPage = sortedPageList.get(nowIndex);
    this.nowNum = 0;
    this.nowIndex = nowIndex;
