...
public class TSDeltaEncoder extends Encoder {
  ...
  public int changeEncode(
      int index, int begPos, long newDelta, TS_DELTA_data compressedData, int endPos) {
    ...
    if (newLen > originalLen) {
      compressedData.rightMoveVals(
          newLen - originalLen, begPos + originalLen, endPos - (newLen - originalLen));
    }
    if (newLen < originalLen) {
  ...
  public int changeEncodeWithFixedLenByInd(
      int index, int begPos, long newDelta, int lenInd, TS_DELTA_data compressedData, int endPos) {
    // 修改在指定位置上的元素的值,数据的整体移动，最多不超过endPos这个位置（不包括endPos）
  ...
  public int changeEncodeWithFixedLen(
      int index, int begPos, long newDelta, int newLen, TS_DELTA_data compressedData, int endPos) {
    // 修改在指定位置上的元素的值,数据的整体移动，最多不超过endPos这个位置（不包括endPos）
    // 新的长度的确定是通过某一个原本位置的元素的长度确定的
    int originalLen = getLen(index, compressedData);
    writeLen(index, newLen, compressedData);
  ...
  public void writeLen(int index, int len, TS_DELTA_data compressedData) {
    ...
    compressedData.lens[index / 4] =
        (byte) (compressedData.lens[index / 4] | (temp << (2 * (3 - index % 4))));
  }
  ...
  private void writeBits(long value, int byteNum, TS_DELTA_data compressedData) {
    ...
    if (byteNum == 2) temp = 2;
    if (byteNum == 4) temp = 3;
    compressedData.lens[valNum / 4] =
        (byte) (compressedData.lens[valNum / 4] | (temp << (2 * (3 - valNum % 4))));
    // 再写上vals数组中的内容
    while (byteNum > 0) {
