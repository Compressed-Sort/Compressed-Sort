...
public abstract class TSEncodingBuilder {
  ...
  public static TSEncodingBuilder getEncodingBuilder(TSEncoding type) {
    switch (type) {
      ...
      case RLBE:
        return new RLBE();
      case DELTA_OF_DELTA:
        return new DELTA_of_DELTA();
      case BACKWARD_DELTA_OF_DELTA:
  ...
  public static class DELTA_of_DELTA extends TSEncodingBuilder {
    @Override
    public Encoder getEncoder(TSDataType type) {
      switch (type) {
        case INT64:
          return new DeltaDeltaLongEncoder();
        default:
          throw new UnSupportedDataTypeException("RLBE doesn't support data type: " + type);
      }
    }
  ...
  public static class BACKWARD_DELTA_of_DELTA extends TSEncodingBuilder {
    @Override
    public Encoder getEncoder(TSDataType type) {
      switch (type) {
        case INT64:
          return new DeltaDeltaBackwardEncoder();
        default:
          throw new UnSupportedDataTypeException("RLBE doesn't support data type: " + type);
      }
    }
  ...
  public static class TS_DELTA extends TSEncodingBuilder {
    @Override
    public Encoder getEncoder(TSDataType type) {
      switch (type) {
        case INT64:
          return new DeltaGorillaEncoder();
        default:
          throw new UnSupportedDataTypeException("RLBE doesn't support data type: " + type);
      }
    }
