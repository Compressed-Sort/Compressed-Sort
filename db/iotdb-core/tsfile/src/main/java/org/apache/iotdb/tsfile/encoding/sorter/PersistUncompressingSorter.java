...
public class PersistUncompressingSorter {
  ...
  public void deletePage(int index, int deleteTimeLen, int deleteValueLen) {
    ...
    while (deleteTimeLen > 0 || deleteValueLen > 0) {
      if (deleteTimeLen > 0 && timeReadPointIndex == 0) {
        if (sortedPageList.get(0).getPageTimeLen() - timeReadPointPos
            <= deleteTimeLen) { // 从时间列考虑可以删除第一个page
          timeReadPointIndex++;
          timeReadPointPos = 0;
      ...
      if (deleteValueLen > 0 && valueReadPointIndex == 0) {
        if (sortedPageList.get(0).getPageValueLen() - valueReadPointPos
            <= deleteValueLen) { // 从数值列考虑可以删除第一个page
          valueReadPointIndex++;
          valueReadPointPos = 0;
  ...
  public void sortPage(CompressedPageData newPage) { // newpage是新加入pagelist里面的元素
    if (newPage.getMinTime() > this.sortedPageMaxTime) {
      ...
      timeChanger.changeEncodeWithFixedLen(
          0,
          0,
          newPage.getMinTime() - this.sortedPageMaxTime,
          8,
          newPage.getTimeData(),
          newPage.getPageTimeLen());
      return;
    }
    CompactionSorterTimeDecoder timeForwardReader =
        new CompactionSorterTimeDecoder(sortedPageList.size() - 1, 0, sortedPageList);
    CompactionSorterTimeDecoder timeBackwardReader =
        new CompactionSorterTimeDecoder(
            sortedPageList.size() - 1, this.sortedPageMaxTime, sortedPageList);
    CompactionSorterValueDecoder valueForwardReader =
        new CompactionSorterValueDecoder(sortedPageList.size() - 1, sortedPageList);
    CompactionSorterValueDecoder valueBackwardReader =
        new CompactionSorterValueDecoder(sortedPageList.size() - 1, sortedPageList);
  ...
  public int persistentSortOpeHoldLen(
      CompressedPageData newPage,
      CompactionSorterTimeDecoder timeForwardDecoder,
      CompactionSorterTimeDecoder timeBackwardDecoder,
      CompactionSorterValueDecoder valueForwardDecoder,
      CompactionSorterValueDecoder valueBackwardDecoder) {
    ...
    int valueTempVar =
        timeChanger.changeEncodeWithFixedLen(
            valueTempNum,
            valueTempValuePos,
            valueTempTime - timeBackwardDecoder.backwardDecode(),
            timeBackwardDecoder.readValueLen(),
            newPage.getTimeData(),
            endTimePosition);
    ...
    if (timeForwardDecoder.getNowValue() > upBound) { // 有newHead,upBound不在new page中
      upBoundVar =
          timeChanger.changeEncodeWithFixedLen(
              ...
              upBoundValuePos,
              upBound - tailTime,
              timeForwardDecoder.readNowValueLen(),
              timeForwardDecoder.getNowValueIndex(),
              timeForwardDecoder.getNowValuePos());
    } else {
      upBoundVar =
          timeChanger.changeEncodeWithFixedLen(
              true,
    ...
    if (timeForwardDecoder.getNowValue() > upBound) {
      ...
      if (timeForwardDecoder.getNowNum() != 0)
        newHeadVar =
            timeChanger.changeEncodeWithFixedLen(
                timeForwardDecoder.getNowNum() - 1,
                newHeadPos,
                timeForwardDecoder.getNowValue() - sortedPageMaxTime,
                8,
                newPage.getTimeData(),
                timeForwardDecoder.getNowValuePos());
      else
    ...
    dataMove(
        ...
        v_upBoundValueIndex,
        v_upBoundValuePos,
        v_valueTempValueIndex,
        v_valueTempValuePos,
        v_newHeadIndex,
        v_newHeadPos);
    lenDataMove(
        true,
        timeBackwardDecoder.getNowIndex(),
        timeBackwardDecoder.getNowNum(),
        valueTempIndex,
        valueTempNum,
        newHeadIndex,
        newHeadLenNum);
    lenDataMove(
        false,
        valueBackwardDecoder.getNowIndex(),
        valueBackwardDecoder.getNowNum(),
        v_valueTempIndex,
        v_valueTempNum,
        v_newHeadIndex,
        v_newHeadLenNum);
  ...
  public void dataMove(
      ...
      int midPos,
      int endIndex,
      int endPos) { // 把mid到end之间的元素移动到beg之前
    dataMoveEndIndex = -1;
    dataMoveBegIndex = -1;
    if (midPos < 0) {
      midIndex--;
  ...
  public void dataExchange(
      boolean isTime, int begIndex, int begPos, int endBegIndex, int endBegPos, int len) {
  ...
  public void lenDataExchange(int pos1, int pos2) {
    ...
    val1 = (byte) (val1 & 0x3);
    val2 = (byte) (val2 & 0x3);
    dataMoveBegVals[pos1 / 4] =
        (byte) ((dataMoveBegVals[pos1 / 4] & (~unmask1)) | (val2 << (2 * (3 - pos1 % 4))));
    dataMoveEndVals[pos2 / 4] =
        (byte) ((dataMoveEndVals[pos2 / 4] & (~unmask2)) | (val1 << (2 * (3 - pos2 % 4))));
  }
