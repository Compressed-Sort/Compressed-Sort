...
public class CompactionSorterTest {
  ...
  public void testOldCompactionMemory() throws Exception {
    ...
    CompactionPage(
        ...
        value2,
        timeBuffer1TS_2DIFF,
        valueBuffer1TS_2DIFF,
        timeBuffer2TS_2DIFF,
        valueBuffer2TS_2DIFF,
        false);
    // CompactionPage(time1, time2, value1, value2, timeBuffer1Chimp, valueBuffer1Chimp,
    // timeBuffer2Chimp, valueBuffer2Chimp, true);
    recordMemory(memoryMXBean);
    time1[0] = 0;
    time2[0] = 0;
    value1[0] = 0;
  ...
  public void testCompactionTime() throws IOException {
    ...
    for (int r = 0; r < REAPEAT_NUM; r++) {
      ...
      while (startIndex + 2 * PAGE_SIZE <= ROW_NUM) {
        ...
        startTime = System.nanoTime();
        CompactionPage(
            timeBuffer1Chimp, valueBuffer1Chimp, timeBuffer2Chimp, valueBuffer2Chimp, true);
    ...
    writeDataToTXT(
        new long[] {
          compactimeNum,
          totalTimeNew / compactimeNum,
          totalTimeTS_2DIFF / compactimeNum,
          totalTimeChimp / compactimeNum
        });
  }
  ...
  public CompressedPageData getPageData(int rowNum, long[] times, long[] values) {
    ...
    return new CompressedPageData(
        ...
        valueCompressedData,
        PAGE_SIZE,
        Arrays.stream(times).min().getAsLong(),
        Arrays.stream(times).max().getAsLong());
  }
  ...
  public void CompactionPage(
      long[] time1,
      long[] time2,
      long[] value1,
      long[] value2,
      ByteBuffer timeBuffer1,
      ByteBuffer valueBuffer1,
      ByteBuffer timeBuffer2,
      ByteBuffer valueBuffer2,
      boolean encoderType)
      throws IOException {
  ...
  public void prepareData(long[] times, long[] values) {
    // 从指定的文件中加载数据集
    //
    // readCSV("D:/senior/DQ/research/compressed_sort_paper/dataset/real/samsung/s-10_cleaned.csv",
    // 2, ROW_NUM+1, 0, times);
    //
    ...
    // readCSV("D:/senior/DQ/research/compressed_sort_paper/dataset/real/swzl/swzl_clean.csv", 2,
    // ROW_NUM+1, 0, times);
    // readCSV("D:/senior/DQ/research/compressed_sort_paper/dataset/real/swzl/swzl_clean.csv", 2,
    // ROW_NUM+1, 1, values);

    readCSV(
        "D:/senior/DQ/research/compressed_sort_paper/dataset/real/ship/shipNet.csv",
        2,
        ROW_NUM + 1,
        1,
        times);
