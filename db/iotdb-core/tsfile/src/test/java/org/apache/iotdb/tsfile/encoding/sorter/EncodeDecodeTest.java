...
public class EncodeDecodeTest {
  ...
  public void testCorrect1() throws IOException {
    ...
    CompressedPageData pageData =
        new CompressedPageData(
            timeCompressedData,
            valueCompressedData,
            ROW_NUM,
            Arrays.stream(times).min().getAsInt(),
            Arrays.stream(times).max().getAsInt());
  ...
  public void testCorrect2() throws IOException {
    ...
    CompressedPageData pageData2 =
        new CompressedPageData(timeCompressedData2, valueCompressedData2, 0, 0, 0);
    data.add(pageData2);

    CompactionSorterTimeDecoder timeDecoder =
        new CompactionSorterTimeDecoder(1, times[ROW_NUM - 1], data);
  ...
  public void testDataMove() {
    for (int j = 0; j < REAPEAT_NUM; j++) {
      ...
      System.arraycopy(
          pageData1.getTimeData().vals, 0, dataToMove, 0, pageData1.getTimeData().vals.length);
      System.arraycopy(
          pageData2.getTimeData().vals,
          0,
          dataToMove,
          pageData1.getTimeData().vals.length,
          pageData2.getTimeData().vals.length);
      ...
      sorter.dataMove(
          true,
          randomInts[0] / pageData1.getPageTimeLen(),
          randomInts[0] % pageData1.getPageTimeLen(),
          randomInts[1] / pageData1.getPageTimeLen(),
          randomInts[1] % pageData1.getPageTimeLen(),
          randomInts[2] / pageData1.getPageTimeLen(),
          randomInts[2] % pageData1.getPageTimeLen());
  ...
  public void testLenDataMove() {
    for (int j = 0; j < REAPEAT_NUM; j++) {
      ...
      PersistUncompressingSorter sorter = new PersistUncompressingSorter(data);
      byte[] dataToMove =
          new byte[pageData1.getTimeData().lens.length + pageData2.getTimeData().lens.length];
      System.arraycopy(
          pageData1.getTimeData().lens, 0, dataToMove, 0, pageData1.getTimeData().lens.length);
      System.arraycopy(
          pageData2.getTimeData().lens,
          0,
          dataToMove,
          pageData1.getTimeData().lens.length,
          pageData2.getTimeData().lens.length);
      ...
      sorter.lenDataMove(
          true,
          randomInts[0] / pageData1.getCount(),
          randomInts[0] % pageData1.getCount(),
          randomInts[1] / pageData1.getCount(),
          randomInts[1] % pageData1.getCount(),
          randomInts[2] / pageData1.getCount(),
          randomInts[2] % pageData1.getCount());
      ...
      for (int i = 0; i < pageData2.getTimeData().lens.length; i++) {
        assertEquals(
            dataToMove[i + pageData1.getTimeData().lens.length], pageData2.getTimeData().lens[i]);
      }
    }
  }
