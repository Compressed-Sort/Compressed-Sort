...
public class PersistUncompressingSorterTest {
  ...
  public void testSort() throws IOException {
    // 数据缩减合并，测试反向解码的效果
    CompressedPageData pageDataOld =
        getPageData(
            new long[] {17001025, 17001030, 17001028, 17001034},
            new long[] {991, -640, 8, 0},
            ROW_NUM,
            LOW_BOUND,
            UP_BOUND);
    CompressedPageData pageDataNew =
        getPageData(
            new long[] {
              -421, -166, 179,
            },
  ...
  public void testSortWithValueRandom() throws IOException {
    for (int j = 0; j < REAPEAT_NUM; j++) {
      ...
      CompressedPageData pageDataOld =
          getPageData(ROW_NUM, LOW_BOUND, UP_BOUND, timesOld, valuesOld);
      CompressedPageData pageDataNew =
          getPageData(ROW_NUM, LOW_BOUND, UP_BOUND, timesNew, valuesNew);
      for (int i = 0; i < ROW_NUM; i++) {
        times[i] = timesOld[i];
        times[ROW_NUM + i] = timesNew[i];
  ...
  public CompressedPageData getPageData(
      int rowNum, long lowBound, long upBound, int[] times, int[] values) {
    for (int i = 0; i < rowNum; i++) {
      values[i] = (int) ((Math.random() * (upBound - lowBound + 1)) + lowBound);
      times[i] = (int) ((Math.random() * (upBound - lowBound + 1)) + lowBound);
    }
  ...
  public CompressedPageData getPageData(
      long[] times, long[] values, int rowNum, long lowBound, long upBound) {
    // Arrays.sort(times);
    TSDeltaEncoder timeEncoder = new TSDeltaEncoder();
    TS_DELTA_data timeCompressedData = new TS_DELTA_data();
    VVarIntEncoder valueEncoder = new VVarIntEncoder();
  ...
  public void prepareData(List<Long> times, List<Long> values) {
    ...
    readCSV(
        "D:\\senior\\毕设\\data\\201306-citibike-tripdata_digital_1000_cleaned.csv",
        1,
        2 * ROW_NUM,
        0,
        times);
    // readCSV("D:\\senior\\毕设\\data\\201306-citibike-tripdata_digital_1000_cleaned.csv",2,
    // ROW_NUM+1, 3, values);
  }
